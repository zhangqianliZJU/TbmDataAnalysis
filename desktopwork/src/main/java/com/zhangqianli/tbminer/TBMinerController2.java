...
public class TBMinerController2 {
    ...
    public void onBirchLaunch() throws Throwable {
    ...
    /**
     * 已经实现按照Timeslice来更新绘图界面的功能，但这个程序性能堪忧，可能跟jvm平台自身性能有关，总之还能继续优化
     */
    public void onBirchLaunch3() {
        final NumberAxis xAxis = new NumberAxis();
        final NumberAxis yAxis = new NumberAxis();
        xAxis.setLabel("时间序列编号");
        ScatterChart<Number, Number> scatterChart = new ScatterChart<>(xAxis, yAxis);
...
//        scatterChart.getStylesheets().add("/tbminer/Chart.css");
        ...
        //这个方法不行的话，直接使用Platform.runlater
        Thread t1 = new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    birch.buildBTree10(scatterChart);
                } catch (Throwable throwable) {
                    throwable.printStackTrace();
                }
            }
