...
//    private int L=5;  //LeafNode的数量上限
    private int L = Params.getL() ;  //LeafNode的数量上限
    private double T = Params.getT();  //MinCluster的直径阈值
    private ArrayList<MinCluster> children;  //LeafNode中包含的MinCluster集合
    private LeafNode pre;  //LeafNode通过双向链表来维护 
    ...
    public void split() {  
        ...
        for (int i = 0; i < len - 1; i++) {  //根据对称性，只要计算矩阵的上三角部分即可。
        ...
        // 以距离最远的孩子节点为中心，把L+1个孩子分为两个大簇。其中一个簇仍留作本节点的孩子，另外一簇需要新创建一个节点来领养它们  
        LeafNode newNode = new LeafNode();  
        newNode.addChild(this.getChildren().get(c2));  
        // 如果本节点已经是Root节点，则需要创建一个新的Root节点  
        if (this.getParent() == null) {  //果然是第一次叶节点分裂时产生的根节点
